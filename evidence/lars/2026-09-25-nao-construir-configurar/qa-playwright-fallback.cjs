...
const out = path.join(root, "evidence", "lars", "2026-09-25-nao-construir-configurar");
const chrome = [
...
const pages = [
  ["post", "posts/2026-09-25-nao-construir-configurar.html"],
  ["index", "index.html"],
];
const viewports = [
  ...
  try {
    for (const [pageName, relative] of pages) {
      for (const [viewportName, viewport, isMobile] of viewports) {
        ...
        if (pageName === "index") await page.locator('#posts a[href="./posts/2026-09-25-nao-construir-configurar.html"]').first().scrollIntoViewIfNeeded();
